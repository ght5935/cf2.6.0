...
class PoiMatchFacePage extends React.Component {
  ...
  componentWillMount() {
    const match = pathToRegexp('/search/new/person2face/:personId').exec(this.props.location.pathname);
  ...
  resolveResult = () => {
    if (this.props.search.poi && this.props.search.poi.personId) {
      this.props.dispatch(routerRedux.push(`/search/new/track/${this.props.search.poi.personId}`))
    }
  };
  ...
  render() {
    return (
      <MayLayout location={this.props.location}>
        <FilterProcessNewView current={1} className={css(styles.process)}/>
