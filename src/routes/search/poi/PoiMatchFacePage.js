...
class PoiMatchFacePage extends React.Component {
  ...
  componentWillMount() {
    const match = pathToRegexp('/search/poi/person2face/:personId').exec(this.props.location.pathname);
    if (match && match[1] && match[1]) {
      ...
      // this.fetchPerson2face({personId: match[1]});
  ...
  resolveResult = () => {
    if (this.props.search.poi && this.props.search.poi.personId) {
      this.props.dispatch(routerRedux.push(`/search/poi/track/${this.props.search.poi.personId}`))
    }
  };
  ...
  render() {
    return (
      <MayLayout location={this.props.location}>
        <FilterProcessPoiView current={1} className={css(styles.process)}/>
