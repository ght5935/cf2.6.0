...
import styles from './FaceBigCardView.css';
 ...
 * @constructor
 */
const FacetrackBigCardView = ({data, onImgClick, className,alarmClass,style,onFaceCardClick,resetImg}) => {
  return (
    <Card className={className ? `${styles.container} ${className}` : styles.container} bodyStyle={{padding: 10}} style={style} onClick={onFaceCardClick ? () => onFaceCardClick(data) : null}>
      ...
      {data.alarmed?<div className={alarmClass?`${alarmClass} ${styles.alarmImg}`:styles.alarmImg}/>:null}
      <div>
        {/*{data.alarmed? <p className={styles.alarmText}>{data.name ? data.name : data.srcName}</p>: <p>{data.name ? data.name : data.srcName}</p>}
*/}
        <p className={styles.fixDate}>{data.name ? data.name : data.srcName}</p>
        <p className={styles.fixDate}>{data.offsetTime ? data.offsetTime : data.captureTime}</p>
      </div>
    </Card>
  )
};
...
FacetrackBigCardView.propTypes = {
...
export default FacetrackBigCardView;
