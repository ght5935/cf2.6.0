...
import {notification, Select, Input, InputNumber, Radio, Upload, Icon, Modal, Tooltip} from 'antd';
...
class NewPoiView extends React.Component {
  ...
  onTagChange(e) {
    ...
    //     if(_this.state.impTag.length >= 3){
    //       if(e.keyCode === 8){
    //         console.log(',,,,,,,,,,,,,,,,,,')
    //         _this.setState({
    //           impTag: _this.state.impTag.substring(0, _this.state.impTag.length-1)
    //         });
    //       }else{
    //         return false;
    //       }
  ...
  onMemoChange(value) {
    ...
    if(this.state.impTag.length >= 50){
      this.setState({
            ...
            <div className={css(styles.person)}>
              ...
              <div className={css(styles.item)}>
                <div className={css(styles.itemText)}>姓名(*)：</div>
                  ...
                  <Input className={css(styles.itemInput)} value={this.state.name} onChange={this.onNameChange}/>
               
              </div>
              <div className={css(styles.item)}>
              <div className={css(styles.itemText)}>性别(*)：</div>
                <RadioGroup className={css(styles.radioGroup)} value={this.state.gender}
              ...
              <div className={css(styles.item)}>
                ...
                <Select className={css(styles.itemInput)} size="large" allowClear value={this.state.groupId} onChange={this.onGroupChange}>
              ...
              <div className={css(styles.item)}>
                ...
                <InputNumber className={css(styles.itemInput)} value={this.state.threshold} min={1} max={99} onChange={this.onThresholdChange}/>
              </div>
              <div className={css(styles.item)}>
                <div className={css(styles.itemText)}>标签：</div>
                <Tooltip placement="topLeft" trigger="focus" title="*输入限制25个字符">
                <Input className={css(styles.itemInput)} value={this.state.impTag} onChange={this.onTagChange}/>
                </Tooltip>
              </div>

              <div className={css(styles.item)}>
                <div className={css(styles.itemText)}>身份证号：</div>
                <Input className={css(styles.itemInput)} value={this.state.identityCard} onChange={this.onIdentityCardChange}/>
              </div>

              <div className={css(styles.item)}>
                <div className={css(styles.itemText)}>备注：</div>
                <Tooltip placement="topLeft" trigger="focus" title="*输入限制50个字符">
                <Input className={css(styles.itemInput)} value={this.state.memo} onChange={this.onMemoChange}/>
                </Tooltip>
              </div>
            </div>
          </div>
